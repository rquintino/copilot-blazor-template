...
## Scope
- Add the `bunit` NuGet package to the existing `BlazorDemo.UnitTests` project (do not create a new test project — keep the tree minimal).
- Add ~5 component tests under `tests/BlazorDemo.UnitTests/Components/`:
...
## Edit zone
- `tests/BlazorDemo.UnitTests/BlazorDemo.UnitTests.csproj` (add `bunit` `<PackageReference>`)
- `tests/BlazorDemo.UnitTests/Components/*.cs` (new)
- `tests/BlazorDemo.UnitTests/xunit.runner.json` (new)
...
- **Prerequisite work already done.** Before writing each test, check whether an equivalent test already exists in the project (`grep -rln 'AddTestAuthorization' tests/BlazorDemo.UnitTests/`). If a test with overlapping intent already exists, skip the duplicate; note in the PR description.
...
## Steps
1. **Verify current state first.** Read the UnitTests csproj, list `tests/BlazorDemo.UnitTests/Components/` (if it exists), check for an existing `xunit.runner.json`, and grep for `bunit` references across the test tree. The snippets below describe the *intent* of each test; apply that intent to whatever is actually in the project today. If a component referenced below has been moved/renamed, find its current location via `grep -rln`. Skip duplicates and note in the PR description.
...
4. Add `tests/BlazorDemo.UnitTests/Components/AuthenticatedLayoutTests.cs` (skip if an equivalent test already exists):
   ```csharp
   using Bunit;
   using Bunit.TestDoubles;
   using BlazorDemo.Web.Components.Layout;
   using Xunit;
...
9. Add `tests/BlazorDemo.UnitTests/xunit.runner.json` if it does not already exist:
   ```json
   {
...
10. Run `dotnet test tests/BlazorDemo.UnitTests`. All new tests should pass in well under a second each.
...
- [ ] `dotnet test tests/BlazorDemo.UnitTests` passes.
